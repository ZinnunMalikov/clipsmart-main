...
class S3Storage:
    ...
    def setup_public_bucket(self):
        """Configure the S3 bucket for public read access to uploaded files"""
        try:
    ...
    def upload_json_output(self, output_data, metadata=None):
        """Upload complete output data as JSON file to S3"""
        try:
    ...
    def upload_latex_output(self, latex_content, metadata=None):
        try:
            ...
            # Store only the final LaTeX content, no extra metadata
            self.s3_client.put_object(
                Bucket=self.bucket_name,
                Key=file_key,
                Body=latex_content,
    ...
    def upload_image_with_latex(self, image_path, latex_content, metadata=None):
        try:
            ...
            # Store only the final LaTeX content
            self.s3_client.put_object(
                Bucket=self.bucket_name,
                Key=latex_key,
                Body=latex_content,
